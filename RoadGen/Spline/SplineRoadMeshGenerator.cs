...
{
    public class SplineRoadMeshGenerator
    {
        public int id;
        public float width;
        ...
        public List<SplineIntersectionMeshGenerator> connectedIntersections = new List<SplineIntersectionMeshGenerator>();
        ...
        public SplineRoadMeshGenerator(int id, float width, float scaleFactor, List<Vector3> points, RoadDataConfig config, Terrain terrain)
        {
            this.id = id;
            this.width = width;
        ...
        public void CalculateVerts()
        {
            ...
            for (int i = 0; i <= roadResolution; i++)
            {
                ...
                float sampleHeight = terrain != null ? terrain.SampleHeight(terrain.transform.TransformPoint(position)) : 0;
        ...
        public RoadMeshData CalculateRoadMeshData()
        {
            ...
            roadMeshData.material = config.roadMatConfig.Where(x => x.laneCount == laneCount).FirstOrDefault().laneMat;
            return roadMeshData;
        }
        ...
        public List<FacilityMarkPoint> CalculateTreeLampPoints(Spline spline)
        {
            ...
            for (int i = 0; i <= resolution; i++)
            {
                ...
                if (roadWidth < 8)
                {
                    type = i % 2 == 0 ? FacilityType.Lamp_2 : FacilityType.Tree_2;
                }
                else if (roadWidth >= 8 && roadWidth < 12)
                {
                    type = i % 2 == 0 ? FacilityType.Lamp_4 : FacilityType.Tree_4;
                }
                else if (roadWidth >= 12 && roadWidth < 18)
                {
                    type = i % 2 == 0 ? FacilityType.Lamp_6 : FacilityType.Tree_6;
                }
                else if (roadWidth >= 18 && roadWidth < 24)
                {
                    type = i % 2 == 0 ? FacilityType.Lamp_8 : FacilityType.Tree_8;
                }
                else
                {
                    type = i % 2 == 0 ? FacilityType.Lamp_10 : FacilityType.Tree_10;
                }
